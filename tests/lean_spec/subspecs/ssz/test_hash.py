...
from lean_spec.types.byte_arrays import ByteList, ByteListBase, ByteVector, ByteVectorBase
...
def test_hash_tree_root_bytevector_48() -> None:
    ...
    bv = ByteVector48(data=bytes(range(48)))
...
# Define explicit Vector types for testing our new approach
class Uint16Vector2(Vector):
...
class FixedVector4(Vector):
    """A vector of exactly 4 Fixed values."""
    ELEMENT_TYPE = None  # Will be set after Fixed is defined
    LENGTH = 4


class VarVector2(Vector):
    """A vector of exactly 2 Var values."""
    ELEMENT_TYPE = None  # Will be set after Var is defined
    LENGTH = 2
...
def test_hash_tree_root_vector_uint16_2() -> None:
    ...
    """
    # Vector of two Uint16 values - using our new explicit class definition
...
class ByteVector48(ByteVectorBase):
...
FixedVector4.ELEMENT_TYPE = Fixed
VarVector2.ELEMENT_TYPE = Var
